...
- spaCy `PhraseMatcher` against a curated skill taxonomy of 1,400+ entries merged from a curated internal list and the pyresparser open-source taxonomy (`data/skills_taxonomy.csv`)
...
LinkedIn and Indeed are the two reliable scrapers in the current deployment. Glassdoor, Google Jobs, and ZipRecruiter scrapers are implemented but hit anti-bot defenses (TLS fingerprinting, session authentication, DOM drift) that require proxy infrastructure or fresh authentication tokens for reliable operation. This is characteristic of production web scraping broadly. Bayt, Naukri, and BDJobs are implemented for international markets and will be reliability-hardened as future work.
...
├── data/
│   └── skills_taxonomy.csv    # 1,400+ skills for NLP matching
...
```mermaid
graph LR
    U["User"] --> FE["Frontend<br/>5 pages · Vanilla JS + Tailwind"]
    FE --> API["FastAPI Backend"]

    API --> PS["platform_selector<br/>60+ region mappings"]
    PS --> JDM["Job Discovery Module"]

    JDM --> TPE["ThreadPoolExecutor<br/>concurrent scraping"]
    TPE --> P1["LinkedIn"]
    TPE --> P2["Indeed"]
    TPE -.-> P3["Glassdoor"]
    TPE -.-> P4["Google Jobs"]
    TPE -.-> P5["ZipRecruiter"]
    TPE -.-> P6["Bayt · Naukri · BDJobs<br/>future hardening"]

    JDM --> SE["skill_extractor<br/>spaCy PhraseMatcher<br/>+ required/preferred<br/>context classifier"]
    SE --> DB[("SQLAlchemy<br/>jobs · sessions ·<br/>resumes · letters")]

    API --> CLGM["Cover Letter Module"]
    CLGM --> RP["resume_parser<br/>3-layer detection<br/>200+ synonyms"]
    RP --> DB
    CLGM --> MS["match_scorer<br/>70/30 weighted<br/>+ alias normalization"]
    MS --> DB
    CLGM --> CL["cover_letter<br/>grounded prompt"]
    CL --> CLAUDE["Claude API"]
    CLAUDE --> CL
```

The system has two modules connected to a shared FastAPI backend and SQL database. The **Job Discovery Module** scrapes multiple US job boards concurrently (LinkedIn and Indeed fully live; Glassdoor, Google, ZipRecruiter implemented with platform-dependent reliability), normalizes to a unified schema, and extracts required/preferred skills via a context-aware classifier. The **Cover Letter Module** parses uploaded resumes once using a 3-layer detection system, computes weighted match scores with alias normalization, and generates tailored cover letters grounded in the candidate's profile via the Claude API.
...
Match scores are a weighted skill overlap: **70% required + 30% preferred**, with alias normalization. They are designed as **relative rankings within a result set**, not absolute fitness measures.

Real-world job descriptions use inconsistent signal language ("required" / "must have" / "preferred"), so the extractor conservatively defaults to **preferred** when no explicit signal is detected within a 600-character context window. This produces scores in the **20–30% range for genuinely relevant roles** and near-zero for irrelevant ones.

The **skill gap** displayed under each job — which required skills the candidate is missing — is the primary actionable output. Absolute percentages should be read as relative indicators: a 25% SWE role ranking above a 2% water-treatment role is the signal that matters.
